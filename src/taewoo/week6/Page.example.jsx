...
export default function Week6Page() {
  ...
  const [viewMode, setViewMode] = useState("card"); // ★ 카드/리스트 토글 ("card" | "list")
  ...
  const [snapping, setSnapping] = useState(false); // ★ slide 애니메이션 중 여부 (transition on/off 제어)
  const [viewportWidth, setViewportWidth] = useState(0); // ★ bannerViewport 실제 너비 (ResizeObserver로 자동 갱신)
  ...
  // ResizeObserver를 쓰면 윈도우 resize 외에도 bannerViewport 자체의 width 변화를 잡아냅니다.
  // [viewMode] 의존성: list ↔ card 전환으로 bannerViewport가 재마운트될 때 옵저버를 다시 붙입니다.
  useEffect(() => {
  ...
  // ★ length 0 가드 추가 — NaN bannerIdx 방지
  const goPrev = () => {
  ...
  /**
   * ★ snapToNext: 부드러운 스와이프의 핵심.
   * 1) 트랙을 한 카드 너비만큼 슬라이드 (transition ON)
   * 2) SNAP_MS 후 데이터 교체 (bannerIdx 변경)
   * 3) 동시에 transition OFF + dragOffset 0으로 instant 리셋 (시각적으로 점프 없음)
   *
   * 사용자 눈에는 "옆 카드가 가운데로 미끄러져 들어옴 → 데이터 교체는 보이지 않음" 흐름.
   */
  ...
  // ★ handleTouchMove 추가: 모바일에서도 손가락을 따라 카드가 움직이도록
  ...
  // baseOffset = 0.2vw - 2×(0.6vw + gap) = -(vw + 2×gap)
  ...
  return (
    <div className={styles["week-page"]}>
      ...
      <div className={styles["sortLabelRow"]}>
        <label className={styles["sortLabel"]} htmlFor="sortPart">
          파트
        </label>
        <select
          name="sortPart"
        ...
        <label className={styles["sortLabel"]} htmlFor="sortType">
          정렬
        </label>
        <select
          name="sortType"
      ...
      {displayList.length === 0 ? (
        ...
        <div className={styles["bannerSection"]}>
          {/* ★ 슬라이더 헤더 — 타이틀 + 보기 모드 토글 (review #22) */}
          ...
          {viewMode === "card" && (
            <>
              <div
                ...
                onTouchMove={handleTouchMove} /* ★ 손가락 따라오기 */
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (!isDragging.current) setSelected(currentMember);
                    }}
                  >
                    <img
                      className={styles["bannerBg"]}
                    ...
                    {/* ★ 전체보기 버튼은 여기서 빠지고 헤더의 viewModeToggle로 이동 (review #22) */}
                  </div>
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.onerror = null;
                      e.currentTarget.src =
                        "https://picsum.photos/seed/fallback/120/120";
                    }}
                  />
        ...
        <div className={styles["pushLion"]} onClick={() => setShowAdd(false)}>
          ...
          >
            <form className={styles["pushLionGrid"]} onSubmit={handleAddSubmit}>
              ...
              <div
                className={`${styles["pushLionRow"]} ${styles["fullWidth"]}`}
              >
                  ...
                  <>
                    ...
                    <ul>
                      ...
                      {selected.contact.website && (
                        <li className={styles["listStyle"]}>
                          Website:{" "}
                          <a href={selected.contact.website.url}>
                            {selected.contact.website.label}
                          </a>
                        </li>
                      )}
                    </ul>
