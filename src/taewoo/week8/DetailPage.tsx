...
export default function DetailPage() {
  ...
  const { memberList } = useOutletContext<OutletContext>();
        ...
        {!member.contact && !member.skills && !member.last ? (
          ...
          <>
            ...
            {member.introduce?.map((text, i) => (
              <p key={`${member.id}-intro-${i}`} className={styles["introduceMyself"]}>
                {text}
              </p>
            ))}

            {(member.contact?.email || member.contact?.phone || member.contact?.website) && (
              <>
                <h3 className={styles["introduceTitle"]}>연락처</h3>
                <ul>
                  {member.contact.email && (
                    <li className={styles["listStyle"]}>Email: {member.contact.email}</li>
                  )}
                  {member.contact.phone && (
                    <li className={styles["listStyle"]}>Phone: {member.contact.phone}</li>
                  )}
