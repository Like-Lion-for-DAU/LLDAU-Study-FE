  ...
  return (
    <ul>
      ...
      {contact.link && (
        <li>
          link :{" "}
          <a href={contact.link.url} target="_blank" rel="noopener noreferrer">
            {contact.link.label}
          </a>
        </li>
      )}
...
export default function WeekPage({}: WeekPageProps) {
  ...
  const fetchRandomUsers = async (count: number, type: FetchType = "add") => {
    ...
    try {
      ...
      if (type === "refresh") {
        const myCards = members.filter((member) => member.isMe);
        setMembers([...myCards, ...newMember]);
      } else {
  ...
  const fillRandomData = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("랜덤 유저 불러오기 실패");
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const newMember: Member = {
      ...
      image: "/lion.png",
      bio: [memberInput.bio],
      skills: memberInput.skills.split(",").map((skill) => skill.trim()),
  ...
  return (
    <div className={styles["week-page"]}>
      ...
      <section className={styles["controlSection"]}>
        <h2 className={styles["controlTitle"]}>멋쟁이사자처럼 아기사자 명단</h2>
        ...
        <div className={styles["filterSection"]}>
          ...
          <select
            value={sortType}
            className={styles["filterControl"]}
            onChange={(e) => updateParam("sort", e.target.value, "recent")}
          >
        ...
        <div className={styles["actionSection"]}>
          ...
          <button
            ...
            className={styles["btnIcon"]}
            onClick={() =>
              fetchRandomUsers(members.filter((m) => !m.isMe).length, "refresh")
            }
          >
            전체 새로고침
          </button>
            ...
            <form onSubmit={handleSubmit} className={styles["formInner"]}>
              ...
              <div className={styles["Width"]}>
                ...
                <input
                  ...
                  onChange={(event) => handleInputChange("skills", event)}
              ...
              <div className={styles["Width"]}>
                ...
                <input
                  ...
                  onChange={(event) => handleInputChange("intro", event)}
                  placeholder="예: 3주차 DOM 조작 연습 중!"
                  required
                />
              </div>
              <div className={styles["Width"]}>
                <label htmlFor="bio" className={styles["text"]}>
                  자기소개 (상세 카드)
                </label>
                <textarea
                  id="bio"
              ...
              <div className={styles["Width"]}>
                <label htmlFor="motto" className={styles["text"]}>
                  한마디
                </label>
                <input
                  id="motto"
                  ...
                  onChange={(event) => handleInputChange("motto", event)}
              ...
              <div className={styles["statsCard"]}>
                ...
                <div className={styles["statsGrid"]}>
                  <div className={styles["statItem"]}>
                    <div className={styles["statValue"]}>{frontendCount}</div>
                    <span>Frontend</span>
                  </div>
                  <div className={styles["statItem"]}>
